...
fun Shop.hasCustomerFrom(city: City): Boolean = customers.map { it.city }.contains(city)
...
// Вернуть сет клиентов, у которых не доставленных заказов больше, чем доставленных
fun Shop.getCustomersWithMoreUndeliveredOrdersThanDelivered(): Set<Customer> = customers.filter { it.orders.count { !it.isDelivered } > it.orders.count { it.isDelivered } }.toSet()
